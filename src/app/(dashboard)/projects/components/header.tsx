...
// import Input from "@/app/(auth)/_components/shared-input";
...
export default function Header({
  ...
  return (
    <div
      className={`md:flex items-center justify-between mb-10 ${
        className ?? ""
      }`}
    >
      <div className={`${titleClassName ?? ""}`}>
        ...
        <p
          className={`text-secondary mt-1 max-w-lg ${
            subTitleClassName ?? ""
          }`}
        >
          {subtitle}
  ...
  <div className="relative w-full sm:w-72">
      ...
      />
          ...
          // <Input
          //    className=" rounded-md focus:outline-none bg-surface-highest"
          //   type="text"
          //   value={searchValue}
          //   leftIcon={
          //     <svg
        ...
        {buttonText && (
          <Button
            ...
            iconClassName="me-2"
            leftIcon={leftIcon}
            className={` rounded-xs ${buttonClassName ?? ""}`}
          >
            <Link href={linkHref ?? ""}>{buttonText}</Link>
          </Button>
        )}
