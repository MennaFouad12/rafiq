...
import StatisticsSkeleton from "@/components/skeletons/stats-skeleton";
...
import { getTasksCalendarStats } from "@/lib/statistics";
import { getAllProjects } from "@/lib/projects";
import { CalendarStatsResponse, Project } from "@/lib/types/statistics";
...
useEffect(() => {
  const fetchStats = async () => {
    try {
      const data = await getTasksCalendarStats(filters);
      setStats(data);
    } catch (err) {
      console.error(err);
    }
...
useEffect(() => {
  ...
  return (
    <>
      ...
      <StatsCard
  completedTasks={stats?.done_tasks ?? 0}
