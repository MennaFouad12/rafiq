...
// import { STATUS_CONFIG_STATS } from "@/lib/constant/tasks.constants";
      ...
      <div className="flex flex-col gap-2 sm:hidden">
        {daily?.map((day) => {
          ...
          return (
            <div
              key={day.day}
              className={`flex items-center justify-between bg-surface-low rounded-lg shadow-sm px-4 py-3 relative ${
                today ? "border-l-4 border-primary bg-white" : ""
              }`}
            >
              <div className="flex flex-col w-12 border-e border-slate-light/30">
              ...
              <div className="flex items-center gap-2 flex-wrap flex-1 ml-4">
                {statusEntries.length === 0 ? (
                  <span className="text-xs text-slate-dark/40">
                    No tasks
                  </span>
                ) : (
                  statusEntries.map(([status, count]) => {
      ...
      <div className="hidden sm:flex justify-between items-stretch">
        {daily?.map((day) => {
          ...
          return (
            <div
              key={day.day}
              className={`flex flex-col min-h-105 min-w-31 p-4 relative bg-white rounded-lg shadow-sm ${
                today ? "border-3 border-primary" : ""
              }`}
            >
              {today && (
                ...
                <div className="flex flex-col gap-2">
                  {Object.entries(day.statuses).map(([status, count]) => {
                    ...
                    return (
                      <div
                        key={status}
                        className={`flex items-center justify-between rounded-xs p-2 text-xs font-bold ${
                          config?.badge ?? "bg-gray-100"
                        } ${config?.text ?? "text-gray-600"}`}
                      >
                        <span>{config?.label ?? status}</span>
