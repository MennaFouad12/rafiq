...
import { fetchEpicTasks, fetchtasks } from "@/redux/features/tasks/task";
...
export default function EpicDetailsModal({
  ...
  const { epicTasks, loadingEpicTasks } = useAppSelector(
    (state) => state.tasks
  );
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-[92%] max-w-xl -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl p-6 shadow-lg">
        ...
        <div>
          <div className="flex justify-between mb-3">
            ...
            <button
              onClick={() => router.push(`/projects/${projectId}/tasks/new?epicId=${epicId}`)}
          ...
          {epicTasks?.length > 0 ? (
            ...
            <ul className="space-y-2 border border-gray-200 ">
              {epicTasks.map((task) => (
                <li
                  key={task.id}
                  className="flex  justify-between items-center border-b border-gray-200  px-4 py-2 rounded"
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="ms-3">
                      ...
                      <p className="flex items-center text-sm"> <Avatar name={task.assignee?.name} /> <span className="pl-3">{task.assignee?.name} </span></p>
                    </div>
                  </div>
